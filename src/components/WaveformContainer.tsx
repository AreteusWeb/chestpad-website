...
// CHANGE (2026-07-20): Resp and PPG now live at fixed indices 8 and 9 in
// the 10-channel `waveforms` array (see useWebSocket.ts header comment).
// Pulling these out as named constants instead of magic numbers so this
// file doesn't silently break again the next time the channel layout changes.
...
const WaveformContainer: React.FC = () => {
  ...
  // CHANGE: use the shared LEADS list from useWebSocket.ts instead of a
  // local hardcoded array. This removes 'Lead III' (the device never sends
  // it) and keeps this file in sync if the lead list ever changes again.
  const leads = LEADS;

  // CHANGE: resolves a lead's display name to its actual ring/waveform
  // index via LEAD_CHANNEL_INDEX, instead of the old `i % 4` trick that
  // only worked with the simulator's 8-channel layout.
